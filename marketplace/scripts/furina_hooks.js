/**
 * Furina hooks script - handles PreToolUse and PostToolUse events
...
/** Extract Furina:*:Purpose stage: matches Furina:\s*([a-zA-Z]+)\s*:Purpose */
const PURPOSE_PATTERN = /Furina:\s*([a-zA-Z]+)\s*:Purpose/i;

/** Extract cwd: matches "cwd"\s*:\s*"([^"]+)" */
const CWD_PATTERN = /"cwd"\s*:\s*"([^"]+)"/i;

/** Extract prompt: matches only /furina:workflow prefix */
...
/** Extract change name from furina change new command */
 ...
 * Only processes when prompt matches /furina:workflow prefix and all
...
/**
 * Extract the change name from an furina change new command string.
...
/**
 * Handle furina change instruction --proposal case:
...
/**
 * Handle furina change archive case:
...
/**
 * Handle --before-bash mode: extract command from rawInput, detect furina commands,
...
export function runBeforeBash(parsed, rawInput) {
  ...
  // Non-furina commands: exit without further processing
  if (!rawCommand.includes('furina')) {
    return;
  }

  // if-else chain dispatch for furina commands
  ...
  // Unmatched furina commands are silently ignored
}
