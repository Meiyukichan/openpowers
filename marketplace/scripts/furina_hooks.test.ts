/**
 * @fileoverview Tests for furina_hooks.js hooks script
...
describe('parseStdin', () => {
  it('should extract session_id and cwd from valid JSON', () => {
    const input = JSON.stringify({
      ...
      tool_input: {
        'Furina:explore:Purpose': 'explore task',
      },
    });
  ...
  it('should not include purpose when no Furina key exists', () => {
    const input = JSON.stringify({
      session_id: 'abc-123',
      cwd: '/tmp/test',
    });
  ...
  it('should extract fields from malformed text with encoding prefix that JSON.parse would reject', () => {
    const input = '\uFEFF{"session_id":"abc-123","tool_input":{"Furina:plan:Purpose":"task"},"cwd":"/tmp/path","trailing":"junk",more:broken}';
  ...
  it('should extract fields even when input is not valid JSON at all', () => {
    const input = 'some_prefix "session_id" : "my-session-id", garbage text "cwd" : "/my/project/path" Furina:coding:Purpose more_noise';
  ...
  it('should handle Chinese characters in cwd path', () => {
    const input = `{"session_id":"abc-123","cwd":"/home/用户/项目/我的代码","tool_input":{"Furina:explore:Purpose":"探索任务"}}`;
  ...
  it('should handle Chinese characters in Windows-style cwd path', () => {
    const input = `{"session_id":"win-001","cwd":"C:\\\\Users\\\\小明\\\\Documents\\\\项目","tool_input":{"Furina:Review:Purpose":"review"}}`;
  ...
  it('should handle JSON-escaped unicode sequences (\\\\uXXXX)', () => {
    const input = `{"session_id":"u-001","cwd":"/tmp/uni","tool_input":{"Furina:plan:Purpose":"\\u8ba1\\u5212\\u4efb\\u52a1"}}`;
  ...
  it('should handle UTF-8 BOM followed by Chinese content', () => {
    const input = '\uFEFF{"session_id":"bom-cn","cwd":"/数据/测试","tool_input":{"Furina:propose:Purpose":"提案"}}';
  ...
  it('should handle raw bytes and non-printable characters around fields', () => {
    const input = '\x00\x01text before\x02"session_id"\t:\n"hex-001"\r\n\x03"cwd":\r"/path/with\\n/newline/value"some junk\x04Furina:workflow:Purpose trailing \x05';
  ...
  it('should handle emoji and special Unicode characters in surrounding text', () => {
    const input = '🎉🚀{"session_id":"emoji-001","cwd":"/home/user/项目🔥","tool_input":{"Furina:plan:Purpose":"📋plan"}}✨🎯';
  ...
  it('should handle extremely long input with fields buried deep', () => {
    ...
    const input = `${noise}"session_id":"deep-999"${noise}"cwd":"/deep/path"${noise}Furina:review:Purpose${noise}`;
  ...
  it('should pick the FIRST match when multiple session_id fields exist', () => {
    const input = '{"session_id":"first-one","cwd":"/path1","tool_input":{"Furina:explore:Purpose":"t1"},"other":{"session_id":"second-one","cwd":"/path2"}}';
  ...
  it('should handle spaces in cwd path (e.g. Windows paths with spaces)', () => {
    const input = `{"session_id":"spc-001","cwd":"C:\\\\Program Files\\\\My App\\\\data","tool_input":{"Furina:finalize:Purpose":"done"}}`;
  ...
  it('should handle session_id with mixed case and return as-is', () => {
    const input = '{"session_id":"AbC-123-XyZ-456","cwd":"/tmp","Furina:explore:Purpose":"x"}';
...
describe('buildBeforeAgentCommand', () => {
  it('should build correct command for --before-agent mode', () => {
    ...
    expect(result).toEqual(['furina', 'agents', 'switch', 'explore', '--session', 'session-001']);
  });

  it('should handle different purpose values', () => {
    const result = buildBeforeAgentCommand('abc-456', 'plan');

    expect(result).toEqual(['furina', 'agents', 'switch', 'plan', '--session', 'abc-456']);
  });
});
...
describe('buildInitCommand', () => {
  it('should build correct init command with session ID and cwd', () => {
    ...
    expect(result).toEqual(['furina', 'agents', 'init', '--session', 'session-003', '--cwd', '/test/cwd']);
  });
...
describe('buildWorkflowCommand', () => {
  it('should build correct command with workflow for --after-agent mode', () => {
    ...
    expect(result).toEqual(['furina', 'agents', 'switch', 'workflow', '--session', 'session-002']);
  });
});
...
describe('buildBeforeProposeCommand', () => {
  it('should build correct command for --before-propose mode', () => {
    ...
    expect(result).toEqual(['furina', 'agents', 'switch', 'propose', '--session', 'abc-123']);
  });
});
...
describe('executeCommand', () => {
  ...
  it('should NOT write to stderr on failure when silent option is true', () => {
    ...
    executeCommand(['furina', 'agents', 'init', '--session', 's', '--cwd', '/tmp'], '/tmp', { silent: true });
...
describe('writeLog', () => {
  ...
  it('should create log directory if it does not exist', () => {
    ...
    expect(mkdirSyncMock).toHaveBeenCalledWith(
      path.join('/mock/home', '.furina', 'logs', 'hooks'),
      { recursive: true },
    );
  });
  ...
  it('should append log line to the correct file with session ID', () => {
    ...
    const logFile = path.join('/mock/home', '.furina', 'logs', 'hooks', 'hooks-my-session-123.log');
...
describe('runAfterAgent', () => {
  ...
  it('should parse prompt/description/toolUseId and write toolResponse file', () => {
    ...
    const expectedPath = path.join('/mock/home', '.furina', 'sessions', 'parse-test', 'tool-abc.json');
...
describe('runBeforeAgent', () => {
  ...
  it('should write log entries for before-agent on success', () => {
    ...
    expect(logLines).toContainEqual(expect.stringContaining('Accepted hook request --- furina-purpose: plan'));
    expect(logLines).toContainEqual(expect.stringContaining('Accepted hook request --- cwd: /test/cwd'));
  });
  ...
  it('should parse prompt/description/tool_use_id via JSON when rawInput is valid JSON', () => {
    ...
    const expectedPath = path.join('/mock/home', '.furina', 'sessions', 'json-test', 'tool-abc-123.txt');
  ...
  it('should create session directory when it does not exist', () => {
    ...
    const expectedDir = path.join('/mock/home', '.furina', 'sessions', 'mkdir-test');
...
describe('runInitAgent', () => {
  ...
  it('should silently return when session_id is missing', () => {
    runInitAgent({
      sessionId: undefined,
      cwd: '/valid/path',
    }, '{"prompt":"/furina:workflow","session_id":"abc-123"}');
  ...
  it('should execute with prompt that has additional content after /furina:workflow', () => {
    execSyncMock.mockReturnValue('init successful');

    runInitAgent({
      sessionId: 'session-xyz',
      cwd: '/another/path',
    }, '{"prompt":"/furina:workflow start new task","session_id":"session-xyz"}');
  ...
  it('should NOT write to stderr when init command fails (execSync throws)', () => {
    ...
    runInitAgent({
      ...
      cwd: '/valid/path',
    }, '{"prompt":"/furina:workflow","session_id":"session-err"}');
  ...
  it('should pass prompt to buildInitCommand via --prompt flag', () => {
    ...
    runInitAgent({
      ...
      cwd: '/valid/project/path',
    }, '{"prompt":"/furina:workflow explore the codebase","session_id":"session-prompt"}');

    expect(execSyncMock).toHaveBeenCalledTimes(2);
    // First call: agents init with --prompt
    const initCallArg = execSyncMock.mock.calls[0][0];
    expect(initCallArg).toContain('--prompt');
    expect(initCallArg).toContain('/furina:workflow explore the codebase');
    // Verify exact command order
    expect(initCallArg).toMatch(/furina agents init --session session-prompt --cwd \/valid\/project\/path --prompt "\/furina:workflow explore the codebase"/);
  });

  it('should NOT include --prompt when prompt is not a workflow prompt', () => {
    // Even with a prompt field, if it doesn't match /furina:workflow, we skip entirely
    runInitAgent({
...
describe('runBeforePropose', () => {
  ...
  it('should write exactly 7 log entries in correct order on happy path', () => {
    ...
    // Entry 2: Accepted hook request --- furina-purpose: propose (hardcoded)
    expect(logLines[1]).toContain('Accepted hook request --- furina-purpose: propose');
...
describe('extractCommandFromRawInput', () => {
  it('should extract command field from rawInput with standard JSON-like format', () => {
    const rawInput = '{"tool_name":"Bash","tool_input":{"command":"furina change new my-feature --desc \\"some description\\"","description":"run command"}}';
    const result = extractCommandFromRawInput(rawInput);
    // JSON.parse correctly unescapes JSON string
    expect(result).toBe('furina change new my-feature --desc "some description"');
  });

  it('should extract command with multi-line content', () => {
    const rawInput = '{"tool_name":"Bash","tool_input":{"command":"furina change new my-feature --desc \\"multi\\nline\\"","description":"run command"}}';
    const result = extractCommandFromRawInput(rawInput);
    // JSON.parse correctly unescapes \n to actual newline and \" to "
    expect(result).toBe('furina change new my-feature --desc "multi\nline"');
  });
  ...
  it('should extract non-furina command', () => {
  ...
  it('should fallback to regex when JSON.parse fails', () => {
    ...
    const rawInput = '\x00BOM{"tool_input":{"command":"furina change new my-feature --desc \\"test\\"","description":"test"}}garbage';
    const result = extractCommandFromRawInput(rawInput);
    expect(result).toBe('furina change new my-feature --desc \\"test\\"');
  });
  ...
  it('should fallback to regex when JSON.parse succeeds but tool_input.command is missing', () => {
    ...
    const rawInput = '{"tool_name":"Bash","tool_input":{"other":"value"},"tool_input_2":{"command":"furina agents list","description":"list"}}';
    const result = extractCommandFromRawInput(rawInput);
    ...
    expect(result).toBe('furina agents list');
  });
});

describe('extractChangeName', () => {
  it('should extract change name from standard change new command', () => {
    const result = extractChangeName('furina change new my-feature --desc "some description"');
    expect(result).toBe('my-feature');
  });

  it('should extract change name with extra flags', () => {
    const result = extractChangeName('furina change new my-feature --desc "desc" --other-flag');
    expect(result).toBe('my-feature');
  });

  it('should return null when command is not change new', () => {
    const result = extractChangeName('furina agents init --session abc');
    expect(result).toBeNull();
  });

  it('should return null for non-furina command', () => {
    const result = extractChangeName('ls -la');
    expect(result).toBeNull();
  });

  it('should extract kebab-case change name', () => {
    const result = extractChangeName('furina change new my-cool-feature --desc "test"');
...
describe('runBeforeBash', () => {
  ...
  it('should silently return when sessionId is missing', () => {
    runBeforeBash({
      ...
      cwd: '/valid/path',
      prompt: undefined,
    }, '{"tool_input":{"command":"furina change new x","description":"d"}}');
  ...
  it('should exit(0) without executing when command is not furina', () => {
    runBeforeBash({
      sessionId: 'abc-123',
      purpose: undefined,
      cwd: '/valid/path',
  ...
  it('should execute agents init with --change when command is furina change new', () => {
    execSyncMock.mockReturnValue('init successful');
    ...
    runBeforeBash({
      ...
      cwd: '/project',
      prompt: undefined,
    }, '{"tool_name":"Bash","tool_input":{"command":"furina change new my-feature --desc \\"some desc\\"","description":"run command"}}');

    expect(execSyncMock).toHaveBeenCalledTimes(1);
    const callArg = execSyncMock.mock.calls[0][0];
    expect(callArg).toContain('furina');
  ...
  it('should write log entries on successful execution', () => {
    ...
    runBeforeBash({
      ...
      cwd: '/project',
      prompt: undefined,
    }, '{"tool_input":{"command":"furina change new feat-x --desc \\"test\\"","description":"d"}}');
  ...
  it('should log error result and not write to stderr when command execution fails', () => {
    ...
    runBeforeBash({
      ...
      cwd: '/project',
      prompt: undefined,
    }, '{"tool_input":{"command":"furina change new bad-feat --desc \\"test\\"","description":"d"}}');
  ...
  it('should NOT trigger handler for change instruction without --proposal', () => {
    ...
    runBeforeBash({
      sessionId: 'design-test',
      cwd: '/project',
    }, '{"tool_input":{"command":"furina change instruction my-change --design","description":"design"}}');
  ...
  it('should call change stage archive for furina change archive command', () => {
    execSyncMock.mockReturnValue('output');

    runBeforeBash({
      sessionId: 'archive-test',
      cwd: '/project',
    }, '{"tool_input":{"command":"furina change archive old-change","description":"archive"}}');
  ...
  it('should silently ignore unmatched furina commands in if-else chain', () => {
    runBeforeBash({
      sessionId: 'unknown-test',
      cwd: '/project',
    }, '{"tool_input":{"command":"furina agents list","description":"list"}}');
...
describe('writeOutputFile', () => {
  ...
  it('should write toolResponse JSON to sessions/<sessionId>/<toolUseId>.json', () => {
    ...
    const expectedPath = path.join('/mock/home', '.furina', 'sessions', 'sess-abc', 'tool-xyz.json');
    expect(writeFileSyncMock).toHaveBeenCalledWith(
      expectedPath,
      JSON.stringify(toolResponse, null, 2),
      'utf-8',
  ...
  it('should create session directory when it does not exist', () => {
    ...
    const expectedDir = path.join('/mock/home', '.furina', 'sessions', 'new-session');
    expect(mkdirSyncMock).toHaveBeenCalledWith(expectedDir, { recursive: true });
    expect(writeFileSyncMock).toHaveBeenCalled();
  });
...
describe('runBeforeQuestion', () => {
  ...
  it('should record questions to question.json when brainstorm is true', () => {
    ...
    const expectedPath = path.join('/mock/home', '.furina', 'sessions', 'test-session', 'question.json');
...
describe('main', () => {
  ...
  it('should handle --after-agent with valid stdin and execute init, workflow switch, and change stage', () => {
    ...
    const stdinJson = JSON.stringify({
      ...
      cwd: '/tmp/test',
      tool_input: {
        'Furina:review:Purpose': 'review task',
      },
    });
    mockStdin(stdinJson);
    execSyncMock.mockReturnValue('output');
  ...
  it('should write log entries with command and full result for before-agent on success', () => {
    ...
    // + 1 Running change stage + 1 Result change stage = 9
    expect(appendFileSyncMock).toHaveBeenCalledTimes(9);
    const logFile = path.join('/mock/home', '.furina', 'logs', 'hooks', 'hooks-log-test-1.log');

    const logLines = appendFileSyncMock.mock.calls.map((call: unknown[]) => call[1]) as string[];
    expect(logLines).toContainEqual(expect.stringContaining('Accepted hook request --- session-id: log-test-1'));
    expect(logLines).toContainEqual(expect.stringContaining('Accepted hook request --- furina-purpose: plan'));
    expect(logLines).toContainEqual(expect.stringContaining('Accepted hook request --- cwd: /test/cwd'));
    expect(logLines).toContainEqual(expect.stringContaining('Running command: furina agents init --session log-test-1 --cwd /test/cwd (cwd: /test/cwd)'));
    ...
    expect(logLines).toContainEqual(expect.stringContaining('Running command: furina agents switch plan --session log-test-1 (cwd: /test/cwd)'));
  ...
  it('should silently return when --init-agent prompt does not match /furina:workflow prefix', () => {
    process.argv = ['node', '/fake/path/script.js', '--init-agent'];
    const stdinJson = JSON.stringify({
      session_id: 'abc-123',
      cwd: '/valid/path',
  ...
  it('should handle --before-bash with furina change new command and execute agents init with --change', () => {
    process.argv = ['node', '/fake/path/script.js', '--before-bash'];
    const stdinJson = '{"tool_name":"Bash","tool_input":{"command":"furina change new my-feature --desc \\"some description\\"","description":"run command"},"session_id":"bash-sess-001","cwd":"/home/user/project"}';
    mockStdin(stdinJson);
    execSyncMock.mockReturnValue('init successful');
  ...
  it('should silently skip when --before-bash command is not furina', () => {
    process.argv = ['node', '/fake/path/script.js', '--before-bash'];
  ...
  it('should silently skip when --before-bash command is furina but not change new', () => {
    process.argv = ['node', '/fake/path/script.js', '--before-bash'];
    const stdinJson = '{"tool_input":{"command":"furina agents list","description":"list agents"},"session_id":"bash-sess-004","cwd":"/home/user/project"}';
    mockStdin(stdinJson);
  ...
  it('should NOT write b.txt debug file when --before-bash mode executes', () => {
    process.argv = ['node', '/fake/path/script.js', '--before-bash'];
    const stdinJson = '{"tool_name":"Bash","tool_input":{"command":"furina change new my-feature --desc \\"some description\\"","description":"run command"},"session_id":"bash-sess-005","cwd":"/home/user/project"}';
    mockStdin(stdinJson);
    execSyncMock.mockReturnValue('init successful');
  ...
  it('should handle --before-question with valid stdin and write questions when brainstorm is true', () => {
    ...
    const expectedPath = path.join('/mock/home', '.furina', 'sessions', 'q-session', 'question.json');
