...
**Prerequisite:** Check if `furina/changes/<name>/plan.json` exists. If not, stop execution and remind the user to first execute the skill `furina-plan` to generate `plan.json`.
...
```bash
furina change feature \<name\> --status
```
...
```bash
furina change feature \<name\> --next
```

### Execute Feature Processing Flow

You **MUST** strictly and accurately execute `Complete Feature Instruction`: `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/instructions/complete-feature.md`

## Feature Management

**Use `furina change feature \<name\>` commands instead of TodoWrite** for persistent, cross-session tracking.

**Why:** TodoWrite is session-scoped and loses state across sessions. The feature list JSON managed by `furina change feature \<name\>` is persistent and serves as the single source of truth.

## Strategy and Benefits

Read the reference document: `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/references/important-matters.md` to learn about the strategy and benefits of the furina-sdd skill.

## Example Workflow

You must first reference the example workflow in `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/references/example-workflow.md` before executing this skill.

## Allowed Documents to Read

**This skill furina-sdd is only allowed to read the following documents; reading any other documents within this skill is illegal:**

- `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/instructions/complete-feature.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/references/example-workflow.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/references/important-matters.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/references/code-implementer-prompt.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/references/specs-reviewer-prompt.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/references/quality-reviewer-prompt.md`
- `${CLAUDE_PLUGIN_ROOT}/skills/furina-sdd/references/reference-explorer-prompt.md`
...
- **furina-plan** — Creates the feature list JSON that this skill executes
- **furina-finalize** — Completes development after all features are done

**Subagents should use:**

- **furina-tdd** — Subagents follow TDD for each feature
