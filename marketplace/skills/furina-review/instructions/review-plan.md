...
**Your task in plan reviewing**:
1. Review the plan in `furina/changes/<name>/plan.json`
...
1. `furina/changes/<name>/plan.json` — Plan JSON (core review target)

### Reference Files (must read for consistency validation)

2. `furina/changes/<name>/proposal.md` — Proposal document
3. `furina/changes/<name>/design.md` — Design document
4. `furina/changes/<name>/specs/**/*.md` — Functional module specs

### Auxiliary Files (read if they exist)

5. `furina/changes/<name>/api.yaml` — API definition
6. `furina/changes/<name>/database.md` — Database design
