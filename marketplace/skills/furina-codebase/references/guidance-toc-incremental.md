...
>
> This specification is fully consistent with `furina-codebase`'s `generate` instruction toc.md generation rules (see `guidance-codebase.md`), ensuring that index files updated via `furina-codebase`'s `synchronize` instruction can be seamlessly queried by `furina-codebase`'s `explore` instruction.
