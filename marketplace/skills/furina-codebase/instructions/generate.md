...
Read Module Plan: `{codebaseDir}/.tmp/module-plan.json`, then process modules in parallel batches of 10 (within each batch, dispatch up to 10 modules concurrently; wait until the current batch completes before starting the next batch):

1. Strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/prompt-submodules-partitioner.md` to dispatch the `SubModules Partitioner Subagent` for ONE module.
...
Process module plans in parallel batches of 10 (within each batch, dispatch up to 10 validators concurrently; wait until the current batch completes before starting the next batch):

1. Strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/prompt-submodules-validator.md` to dispatch the `SubModules Validator Subagent` for ONE module plan - `{codebaseDir}/.tmp/module-{name}-plan.json`.
...
Before entering Phase 5, strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/prompt-codebase-toptoc.md` to dispatch the `Init-Top-Toc Subagent` to generate the initial version of `{codebaseDir}/toc.md`.
...
1. Strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/prompt-codebase-spec.md` to dispatch the `Spec Generator Subagent` for ONE spec
...
After all spec documents under a submodule are written, strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/prompt-codebase-submoduletoc.md` to dispatch the `Submodule Toc Subagent` for ONE submodule.

For each completed submodule toc.md, output progress: `[toc] module/submodule/toc.md — done`

#### 5.5 Create Module Index File After Processing All Module Children

After all child items (submodules and direct specs) under a module are processed, strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/prompt-codebase-moduletoc.md` to dispatch the `Module Toc Subagent` for ONE module.

For each completed module toc.md, output progress: `[toc] module/toc.md — done`

#### 5.6 Update Overview Document After All Modules Are Processed

After all modules are processed, rewrite `{codebaseDir}/toc.md` with complete module descriptions, submodule descriptions, spec descriptions, and module index paths using template: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/template-top-toc.md`
...
After all spec and index documents are generated, strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/prompt-codebase-reviewer.md` to dispatch the `Comprehensive Reviewer Subagent` to perform a comprehensive review and optimization of the entire project documentation.

#### 5.9 Final Check — Verify Index Traceability

After completing all document generation and comprehensive review, strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/prompt-codebase-checker.md` to dispatch the `Index Traceability Checker Subagent` to perform index traceability verification on all toc.md files to ensure that starting from any index level, the final spec and source code can be smoothly located.
...
2. **Quality Review**:
   - For each suspected substandard spec, re-read `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/guidance-specs.md` for comparison
...
3. **Source File List Check**:
   ...
   - Check whether it contains a source file list (format reference: `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/guidance-specs.md` document header metadata)
...
4. **Fix Processing**:
   - For specs determined to be substandard, re-read the corresponding source files and regenerate according to `${CLAUDE_PLUGIN_ROOT}/skills/furina-codebase/references/guidance-specs.md`
...
6.  **Progress reporting.** After completing each item, output progress with `[module]`, `[submodule]`, `[spec]`, `[toc]` prefixes.
...
12.  **Skip test files.** Any files matching `*.test.ts`, `*.test.js`, `*.spec.ts`, `*.spec.js`, `__tests__/**`, `**/test/**`, `**/tests/**` and similar test-related patterns are skipped entirely, no spec documents generated, not included in the Codebase.