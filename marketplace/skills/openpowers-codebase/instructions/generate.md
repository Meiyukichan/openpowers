...
After all spec documents under a submodule are written, strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/openpowers-codebase/references/prompt-codebase-submoduletoc.md` to dispatch the `Submodule Toc Subagent` for ONE submodule.
...
After all child items (submodules and direct specs) under a module are processed, strictly follow the template: `${CLAUDE_PLUGIN_ROOT}/skills/openpowers-codebase/references/prompt-codebase-moduletoc.md` to dispatch the `Module Toc Subagent` for ONE module.
...
2. **Quality Review**:
   - For each suspected substandard spec, re-read `${CLAUDE_PLUGIN_ROOT}/skills/openpowers-codebase/references/guidance-specs.md` for comparison
...
3. **Source File List Check**:
   ...
   - Check whether it contains a source file list (format reference: `${CLAUDE_PLUGIN_ROOT}/skills/openpowers-codebase/references/guidance-specs.md` document header metadata)
...
4. **Fix Processing**:
   - For specs determined to be substandard, re-read the corresponding source files and regenerate according to `${CLAUDE_PLUGIN_ROOT}/skills/openpowers-codebase/references/guidance-specs.md`
