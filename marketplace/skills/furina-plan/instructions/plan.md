...
Before writing the plan, you MUST first break down the implementation work into a task list based on the Furina artifacts you read, following this template:
...
**RED LAW**:
- plan.json MUST NOT contain more than `feature factor` × (number of spec files in `furina/changes/<name>/specs/`) features (minimum of features: 1).
...
| `spec_refs` | yes | References to upstream specification documents. Must accurately point to relevant parts of specs under `specs/`, or to other artifacts like `design.md`, `api.yaml`, `database.md`. Use anchors or line references where applicable (e.g., `furina/changes/<name>/specs/auth/spec.md#login`, `furina/changes/<name>/design.md#authentication-flow`). |
...
- `furina/changes/<name>/plan.json`

## Completion

After saving the plan, inform the user that all preparation is complete and they can use `furina-sdd` to execute the change.