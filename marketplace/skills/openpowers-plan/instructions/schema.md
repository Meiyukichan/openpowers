...
1. **Understand** - Read and analyze all three OpenPowers artifacts (proposal, design, specs) in `openpowers/changes/<name>/`. If any missing, stop and ask user to run `openpowers-propose` first.
2. **Selections** - Carefully select schemas to be created for the current change `openpowers/changes/<name>/` based on `Schema Selection Advice`.
...
**Database**:
1. WHEN and ONLY when current change `openpowers/changes/<name>/` volves the database or SQL

The above is a geneal adivice. The final decision bases on the design docs and specs in `openpowers/changes/<name>/`.
