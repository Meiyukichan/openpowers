...
3. Invoke the `furina-cleancode` skill with the appropriate parameters
...
### Phase 3: Invoke the furina-cleancode Skill

Call the skill `furina-cleancode` with the following arguments:
   ...
   - `outputFile`: None (forbid `furina-cleancode` to generate output file)
...
```md
Furina Explore — Exploration Result
# Explore Content
...
2. **Do not fabricate standards** – All rules must come from the `furina-cleancode` skill. Do not invent conventions.
